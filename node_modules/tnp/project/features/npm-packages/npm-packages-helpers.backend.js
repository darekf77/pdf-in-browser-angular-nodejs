...
function copyMainProjectDependencies(projects, tmpProject, project, pkg) {
    ...
    function copyOtherProcess(parent) {
        ...
        otherDepsInTemp
            ...
            return tnp_core_2.fse.existsSync(tnp_core_1.path.join(tmpProject.node_modules.path, otherDependenyInTemp.name));
        })
            .forEach(function (otherDependenyInTemp) {
            var existedPkgPath = tnp_core_1.path.join(project.node_modules.path, otherDependenyInTemp.name);
            ...
            if (existedOtherDependency) {
                if (existedOtherDependency.version === otherDependenyInTemp.version) {
                    ...
                    if (parent.packageJson.checDepenciesAreSatisfyBy(existedOtherDependency)) {
                        ...
                        var dest = tnp_core_1.path.join(project.node_modules.path, mainProjectExisted.name, tnp_config_1.config.folder.node_modules, otherDependenyInTemp.name);
...
function copyMainProject(tmpProject, project, pkg) {
    var mainProjectInTemp = abstract_1.Project.From(tnp_core_1.path.join(tmpProject.node_modules.path, pkg.name));
    var mainProjectExistedPath = tnp_core_1.path.join(project.node_modules.path, pkg.name);
...
function prepareTempProject(project, pkg) {
    ...
    var tmpFolder = tnp_core_1.path.join(project.location, pathPart + "-" + pkg.name.replace('/', '-') + "-" + tnp_core_3._.snakeCase(pkg.version ? pkg.version : ''));
    tnp_helpers_1.Helpers.remove(tnp_core_1.path.join(project.location, pathPart) + "*");
...
function fixOptions(options) {
    if (tnp_core_3._.isNil(options)) {
        options = {};
    }
    if (tnp_core_3._.isUndefined(options.generatLockFiles)) {
        options.generatLockFiles = false;
    }
    if (tnp_core_3._.isUndefined(options.useYarn)) {
        options.useYarn = false;
    }
    if (tnp_core_3._.isUndefined(options.remove)) {
        options.remove = false;
    }
    if (tnp_core_3._.isUndefined(options.smoothInstall)) {
        options.smoothInstall = false;
    }
    if (tnp_core_3._.isUndefined(options.pkg)) {
        options.pkg = void 0;
    }
    if (tnp_core_3._.isUndefined(options.reason)) {
...
function fixOptionsNpmInstall(options, project) {
    ...
    if (!tnp_core_3._.isArray(options.npmPackages)) {
        options.npmPackages = [];
    }
    if (tnp_core_3._.isUndefined(options.remove)) {
        options.remove = false;
